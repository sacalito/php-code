...
$xyz_snippets_arr=$wpdb->get_results($wpdb->prepare( "SELECT id,title FROM ".$wpdb->prefix."php_code WHERE status=%d ORDER BY title ASC",1),ARRAY_A );
               ...
               ed.addButton( '<?php echo $buttonName;?>', {
		            ...
		            menu: [
			           		...
			           		<?php foreach ($xyz_snippets_arr as $key=>$val) { ?>      
			            		...
			            		value: '[php-code snippet="<?php echo addslashes($val['title']); ?>"]',
			            		onclick: function() {
...
<?php	
	...
	$xyz_snippets = array(
                'title'   =>'PHP Code',
				'url'	=> plugins_url('images/logo.png', PHP_CODE_ROOT),
                'php_code_snippets' => $xyz_snippets_arr
            );
	?>
	...
	tinymce.create('tinymce.plugins.<?php echo $buttonName; ?>', {
		...
		init : function(ed, url) {
         ...
         tinymce_<?php echo $buttonName; ?>.insert = function(){
                if(this.v && this.v != ''){
                tinymce.execCommand('mceInsertContent', false, '[php-code snippet="'+tinymce_<?php echo $buttonName; ?>.php_code_snippets[this.v]['title']+'"]');
				}
            };
		...
		createControl : function(n, cm) {
			if(n=='<?php echo $buttonName; ?>'){
				...
				c.onRenderMenu.add(function(c, m){
		                 for (var id in tinymce_<?php echo $buttonName; ?>.php_code_snippets){
                            m.add({
                                v : id,
                                title : tinymce_<?php echo $buttonName; ?>.php_code_snippets[id]['title'],
                                onclick : tinymce_<?php echo $buttonName; ?>.insert
                            });
                        }
                    });
