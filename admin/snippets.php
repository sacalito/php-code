	...
	if (isset($_POST['apply_ips_bulk_actions'])){
		if (isset($_POST['ips_bulk_actions_snippet'])){
			...
			if (isset($_POST['xyz_ips_snippet_ids']))
				...
				{
					header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=8&pagenum='.$xyz_ips_pageno));
					exit();
				}
				if ($ips_bulk_actions_snippet==2)//bulk-delete
				{
					...
					{
						$wpdb->query($wpdb->prepare( 'DELETE FROM  '.$wpdb->prefix.'php_code  WHERE id=%d',$snippet_id)) ;
					}
					header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=3&pagenum='.$xyz_ips_pageno));
					exit();
				}
				elseif ($ips_bulk_actions_snippet==0)//bulk-Deactivate
				{
					foreach ($xyz_ips_snippet_ids as $xyz_ips_snippetId)
						$wpdb->update($wpdb->prefix.'php_code', array('status'=>2), array('id'=>$xyz_ips_snippetId));
						header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=4&pagenum='.$xyz_ips_pageno));
						exit();
				}
				elseif ($ips_bulk_actions_snippet==1)//bulk-activate
				{
					foreach ($xyz_ips_snippet_ids as $xyz_ips_snippetId)
						$wpdb->update($wpdb->prefix.'php_code', array('status'=>1), array('id'=>$xyz_ips_snippetId));
						header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=4&pagenum='.$xyz_ips_pageno));
						exit();
				}
				elseif ($ips_bulk_actions_snippet==-1)//no action selected
				{
					header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=7&pagenum='.$xyz_ips_pageno));
					exit();
				}
		}
...
<div >
	...
	<?php wp_nonce_field( 'bulk_actions_ips');?>
		<fieldset
			...
			$limit = get_option('php_code_limit');
			$offset = ( $pagenum - 1 ) * $limit;


			$field=get_option('php_code_sort_field_name');
			$order=get_option('php_code_sort_order');
			...
			$entries = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."php_code WHERE title like '%".$search_name_db."%'"." ORDER BY  $field $order LIMIT $offset,$limit" );
...
<table class="widefat" style="width: 99%; margin: 0 auto; border-bottom:none;">
	...
	<tbody>
		...
		<td><?php echo $entry->id ?></td>
			...
			echo '[php-code snippet="'.esc_html($entry->title).'"]';
			?></td>
			...
			<?php
					if($entry->status == 2){
						$stat1 = admin_url('admin.php?page=php-code-manage&action=snippet-status&snippetId='.$snippetId.'&status=1&pageno='.$pagenum);
			?>
			<td style="text-align: center;"><a
				href='<?php echo wp_nonce_url($stat1,'ips-pstat_'.$snippetId); ?>'><img
					id="xyz_ips_img" title="Activate"
					src="<?php echo plugins_url('images/activate.png',PHP_CODE_ROOT)?>">
			</a>
			</td>
				<?php
					}elseif ($entry->status == 1){
						$stat2 = admin_url('admin.php?page=php-code-manage&action=snippet-status&snippetId='.$snippetId.'&status=2&pageno='.$pagenum);
					?>
			<td style="text-align: center;"><a
				href='<?php echo wp_nonce_url($stat2,'ips-pstat_'.$snippetId); ?>'><img
					id="xyz_ips_img" title="Deactivate"
					src="<?php echo plugins_url('images/pause.png',PHP_CODE_ROOT)?>">
			</a>
			</td>
					<?php
					}
			...
			<td style="text-align: center;"><a
				href='<?php echo admin_url('admin.php?page=php-code-manage&action=snippet-edit&snippetId='.$snippetId.'&pageno='.$pagenum); ?>'><img
					id="xyz_ips_img" title="Edit Snippet"
					src="<?php echo plugins_url('images/edit.png',PHP_CODE_ROOT)?>">
			</a>
			</td>

			<?php $delurl = admin_url('admin.php?page=php-code-manage&action=snippet-delete&snippetId='.$snippetId.'&pageno='.$pagenum);?>
			<td style="text-align: center;" ><a
				...
				onclick="javascript: return confirm('Please click \'OK\' to confirm ');"><img
					...
					src="<?php echo plugins_url('images/delete.png',PHP_CODE_ROOT)?>">
			</a></td>
			...
			<td style="text-align: center;" >
				<a href='<?php echo $prewurl;?>' target="_blank">
					<img id="xyz_ips_img" title="Preview" src="<?php echo plugins_url('images/preview.png',PHP_CODE_ROOT)?>">
				</a>
			</td>
		</tr>
		<?php
		...
		<?php } ?>
			...
			<br>
			<input id="xyz_ips_submit_ips" style="cursor: pointer; margin-top:10px;margin-left:8px;" type="button"	name="textFieldButton2" value="Add New PHP Code Snippet" onClick='document.location.href="<?php echo admin_url('admin.php?page=php-code-manage&action=snippet-add');?>"'></div>

			<?php
			$total = $wpdb->get_var( "SELECT COUNT(`id`) FROM ".$wpdb->prefix."php_code" );
