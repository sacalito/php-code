...
if(isset($_POST) && isset($_POST['addSubmit'])){
    ...
    if($xyz_ips_title != "" && $xyz_ips_content != ""){
        if(ctype_alnum($temp_xyz_ips_title)){
            $snippet_count = $wpdb->query($wpdb->prepare( 'SELECT * FROM '.$wpdb->prefix.'php_code WHERE title=%s',$xyz_ips_title) ) ;
            if($snippet_count == 0){
              ...
              if(get_option('php_code_auto_insert')==1){
                ...
                $wpdb->insert($wpdb->prefix.'php_code', array('title' =>$xyz_ips_title,'content'=>$xyz_ips_content,'short_code'=>$xyz_shortCode,'status'=>'1'),array('%s','%s','%s','%d'));
                header("Location:".admin_url('admin.php?page=php-code-manage&xyz_ips_msg=1'));
            }
            else{
?>
