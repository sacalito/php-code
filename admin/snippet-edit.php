...
if(get_option('php_code_auto_insert')==1){
    ...
    if($xyz_ips_title != "" && $xyz_ips_content != ""){
        if(ctype_alnum($temp_xyz_ips_title)){
            $snippet_count = $wpdb->query($wpdb->prepare( 'SELECT * FROM '.$wpdb->prefix.'php_code WHERE id!=%d AND title=%s LIMIT 0,1',$xyz_ips_snippetId,$xyz_ips_title)) ;
            if($snippet_count == 0){
                $xyz_shortCode = '[php-code snippet="'.$xyz_ips_title.'"]';
                $wpdb->update($wpdb->prefix.'php_code', array('title'=>$xyz_ips_title,'content'=>$xyz_ips_content,'short_code'=>$xyz_shortCode,), array('id'=>$xyz_ips_snippetId));
                header("Location:".admin_url('admin.php?page=php-code-manage&action=snippet-edit&snippetId='.$xyz_ips_snippetId.'&xyz_ips_msg=1'.'&goback='.$goback));
            }
            else{
?>
